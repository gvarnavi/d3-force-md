...
<a name="forceSimulation" href="#forceSimulation">#</a> d3.<b>forceSimulation</b>([<i>nodes</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_restart" href="#simulation_restart">#</a> <i>simulation</i>.<b>restart</b>() · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_stop" href="#simulation_stop">#</a> <i>simulation</i>.<b>stop</b>() · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_tick" href="#simulation_tick">#</a> <i>simulation</i>.<b>tick</b>([<i>iterations</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_dt" href="#simulation_dt">#</a> <i>simulation</i>.<b>dt</b>([<i>dt</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_nodes" href="#simulation_nodes">#</a> <i>simulation</i>.<b>nodes</b>([<i>nodes</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_alpha" href="#simulation_alpha">#</a> <i>simulation</i>.<b>alpha</b>([<i>alpha</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_alphaMin" href="#simulation_alphaMin">#</a> <i>simulation</i>.<b>alphaMin</b>([<i>min</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_alphaDecay" href="#simulation_alphaDecay">#</a> <i>simulation</i>.<b>alphaDecay</b>([<i>decay</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_alphaTarget" href="#simulation_alphaTarget">#</a> <i>simulation</i>.<b>alphaTarget</b>([<i>target</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_velocityDecay" href="#simulation_velocityDecay">#</a> <i>simulation</i>.<b>velocityDecay</b>([<i>decay</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_force" href="#simulation_force">#</a> <i>simulation</i>.<b>force</b>(<i>name</i>[, <i>force</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_find" href="#simulation_find">#</a> <i>simulation</i>.<b>find</b>(<i>x</i>, <i>y</i>[, <i>radius</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="simulation_randomSource" href="#simulation_randomSource">#</a> <i>simulation</i>.<b>randomSource</b>([<i>source</i>]) [<>](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js "Source"))
...
<a name="simulation_on" href="#simulation_on">#</a> <i>simulation</i>.<b>on</b>(<i>typenames</i>, [<i>listener</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
A *force* is simply a function that modifies nodes’ positions, velocities, and forces; in this context, a *force* can apply a classical physical force such as electrical charge or gravity, or it can resolve a geometric constraint, such as keeping nodes within a bounding box or keeping linked nodes a fixed distance apart. For example, a simple positioning force that moves nodes towards the origin ⟨0,0⟩ might be implemented as:
...
Forces typically read the node’s current position ⟨*x*,*y*⟩ and then compute the forces experienced by the node ⟨*force_x*,*force_y*⟩. Once all forces acting on nodes have been accumulated, the [simulation](#forceSimulation) updates the forces and velocities according to the equations:

<img alt="Velocity-Verlet-Equations" src="https://latex.codecogs.com/svg.latex?%5Cinline%20%5CLARGE%20%5Cbegin%7Balign%7D%20%5Cnotag%20v_%7Bn&plus;1/2%7D%20%26%3D%20v_n%20&plus;%20f_n%20%5Cfrac%7B%5CDelta%20t%7D%7B2%20m%7D%20%5C%5C%20%5Cnotag%20r_%7Bn&plus;1%7D%20%26%3D%20r_n%20&plus;%20v_%7Bn&plus;1/2%7D%20%5C%5C%20%5Cnotag%20v_%7Bn&plus;1%7D%20%26%3D%20v_%7Bn&plus;1/2%7D%20&plus;%20f_%7Bn&plus;1%7D%20%5Cfrac%7B%5CDelta%20t%7D%7B2%20m%7D%20%5Cend%7Balign%7D" height="125">
...
For backwards-compatibility with [d3-force](https://github.com/d3/d3-force) (version [2.1.1](https://github.com/d3/d3-force/tree/v2.1.1)), some of the built-in forces may also read the node’s current position ⟨*x*,*y*⟩ and then add to (or subtract from) the node’s velocity ⟨*vx*,*vy*⟩, or even “peek ahead” to the anticipated next position of the node, ⟨*x* + *vx*,*y* + *vy*⟩; this is necessary for resolving geometric constraints through [iterative relaxation](https://en.wikipedia.org/wiki/Relaxation_\(iterative_method\)). Built-in forces which does this are marked with an asterisk above.
...
<a name="_force" href="#_force">#</a> <i>force</i>(<i>alpha</i>) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="force_initialize" href="#force_initialize">#</a> <i>force</i>.<b>initialize</b>(<i>nodes</i>) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/simulation.js)
...
<a name="forceCenter" href="#forceCenter">#</a> d3.<b>forceCenter</b>([<i>x</i>, <i>y</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/center.js)
...
<a name="center_x" href="#center_x">#</a> <i>center</i>.<b>x</b>([<i>x</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/center.js)
...
<a name="center_y" href="#center_y">#</a> <i>center</i>.<b>y</b>([<i>y</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/center.js)
...
<a name="center_strength" href="#center_strength">#</a> <i>center</i>.<b>strength</b>([<i>strength</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/center.js), [Examples](https://observablehq.com/@d3/forcecenter-strength)
...
<a name="forceCollide" href="#forceCollide">#</a> d3.<b>forceCollide</b>([<i>radius</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/collide.js)
...
<a name="collide_radius" href="#collide_radius">#</a> <i>collide</i>.<b>radius</b>([<i>radius</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/collide.js)
...
<a name="collide_strength" href="#collide_strength">#</a> <i>collide</i>.<b>strength</b>([<i>strength</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/collide.js)
...
<a name="collide_iterations" href="#collide_iterations">#</a> <i>collide</i>.<b>iterations</b>([<i>iterations</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/collide.js)
...
<a name="forceLink" href="#forceLink">#</a> d3.<b>forceLink</b>([<i>links</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/link.js)
...
<a name="link_links" href="#link_links">#</a> <i>link</i>.<b>links</b>([<i>links</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/link.js)
...
<a name="link_id" href="#link_id">#</a> <i>link</i>.<b>id</b>([<i>id</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/link.js)
...
<a name="link_distance" href="#link_distance">#</a> <i>link</i>.<b>distance</b>([<i>distance</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/link.js)
...
<a name="link_strength" href="#link_strength">#</a> <i>link</i>.<b>strength</b>([<i>strength</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/link.js)
...
<a name="link_iterations" href="#link_iterations">#</a> <i>link</i>.<b>iterations</b>([<i>iterations</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/link.js)

If *iterations* is specified, sets the number of iterations per application to the specified number and returns this force. If *iterations* is 0, the [force](#forceLink) performs a single energy-conserving iteration (useful for molecular dynamics simulations). If *iterations* > 0, the [force](#forceLink) instead performs [iterative relaxation](https://en.wikipedia.org/wiki/Relaxation_(iterative_method)). If *iterations* is not specified, returns the current iteration count which defaults to 1.
...
<a name="forceManyBody" href="#forceManyBody">#</a> d3.<b>forceManyBody</b>() · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/manyBody.js)

Creates a new many-body force with the default parameters.

<a name="manyBody_strength" href="#manyBody_strength">#</a> <i>manyBody</i>.<b>strength</b>([<i>strength</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/manyBody.js)
...
<a name="manyBody_theta" href="#manyBody_theta">#</a> <i>manyBody</i>.<b>theta</b>([<i>theta</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/manyBody.js)
...
<a name="manyBody_distanceMin" href="#manyBody_distanceMin">#</a> <i>manyBody</i>.<b>distanceMin</b>([<i>distance</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/manyBody.js)

If *distance* is specified, sets the minimum distance between nodes over which this force is considered. If *distance* is not specified, returns the current minimum distance, which defaults to 1. A minimum distance establishes an upper bound on the strength of the force between two nearby nodes, avoiding instability. In particular, it avoids an infinitely-strong force if two nodes are exactly coincident; in this case, the direction of the force is random.

<a name="manyBody_distanceMax" href="#manyBody_distanceMax">#</a> <i>manyBody</i>.<b>distanceMax</b>([<i>distance</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/manyBody.js)
...
<a name="forceX" href="#forceX">#</a> d3.<b>forceX</b>([<i>x</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/x.js)

Creates a new positioning force along the *x*-axis towards the given position [*x*](#x_x). If *x* is not specified, it defaults to 0.

<a name="x_strength" href="#x_strength">#</a> <i>x</i>.<b>strength</b>([<i>strength</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/x.js)
...
<a name="x_x" href="#x_x">#</a> <i>x</i>.<b>x</b>([<i>x</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/x.js)
...
<a name="forceY" href="#forceY">#</a> d3.<b>forceY</b>([<i>y</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/y.js)

Creates a new positioning force along the *y*-axis towards the given position [*y*](#y_y). If *y* is not specified, it defaults to 0.

<a name="y_strength" href="#y_strength">#</a> <i>y</i>.<b>strength</b>([<i>strength</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/y.js)
...
<a name="y_y" href="#y_y">#</a> <i>y</i>.<b>y</b>([<i>y</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/y.js)
...
<a name="forceRadial" href="#forceRadial">#</a> d3.<b>forceRadial</b>(<i>radius</i>[, <i>x</i>][, <i>y</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/radial.js)

[<img alt="Radial Force" src="https://raw.githubusercontent.com/gvarnavi/d3-force-md/master/img/radial.png" width="420" height="219">](https://bl.ocks.org/mbostock/cd98bf52e9067e26945edd95e8cf6ef9)
...
<a name="radial_strength" href="#radial_strength">#</a> <i>radial</i>.<b>strength</b>([<i>strength</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/radial.js)
...
<a name="radial_radius" href="#radial_radius">#</a> <i>radial</i>.<b>radius</b>([<i>radius</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/radial.js)
...
<a name="radial_x" href="#radial_x">#</a> <i>radial</i>.<b>x</b>([<i>x</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/radial.js)
...
<a name="radial_y" href="#radial_y">#</a> <i>radial</i>.<b>y</b>([<i>y</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/radial.js)
...
The [Lennard-Jones](https://en.wikipedia.org/wiki/Lennard-Jones_potential) (or *nm*-potential, *Mie*-potential) force is a common many-body force in molecular dynamics which applies mutually amongst all [nodes](#simulation_nodes). This implementation considers a general class of Lennard-Jones potentials with varying exponents to model repulsive and attractive forces, and uses quadtrees and the [Barnes–Hut approximation](https://en.wikipedia.org/wiki/Barnes–Hut_simulation) to greatly improve performance; the accuracy can be customized using the [theta](#manyBody_theta) parameter.

Unlike links, which only affect two linked nodes, the interatomic potential force is global: every node affects every other node, even if they are on disconnected subgraphs.

<a name="forceLennardJonesPotential" href="#forceLennardJones">#</a> d3.<b>forceLennardJonesPotential</b>() · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/lennardJonesPotential.js)

Creates a new lennard-jones force with the default parameters.

<a name="lennardJonesPotential_strength" href="#lennardJonesPotential_strength">#</a> <i>lennardJonesPotential</i>.<b>strength</b>([<i>strength</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/lennardJonesPotential.js)

If *strength* is specified, sets the strength accessor to the specified number or function, re-evaluates the strength accessor for each node, and returns this force. Currently, strength merely acts a counter accumulated in the Barnes-Hut implementation. If *strength* is not specified, returns the current strength accessor, which defaults to:
...
<a name="lennardJonesPotential_theta" href="#lennardJonesPotential_theta">#</a> <i>lennardJonesPotential</i>.<b>theta</b>([<i>theta</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/lennardJonesPotential.js)

If *theta* is specified, sets the Barnes–Hut approximation criterion to the specified number and returns this force. If *theta* is not specified, returns the current value, which defaults to 0.25.

To accelerate computation, this force implements the [Barnes–Hut approximation](http://en.wikipedia.org/wiki/Barnes–Hut_simulation) which takes O(*n* log *n*) per application where *n* is the number of [nodes](#simulation_nodes). For each application, a [quadtree](https://github.com/d3/d3-quadtree) stores the current node positions; then for each node, the combined force of all other nodes on the given node is computed. For a cluster of nodes that is far away, the interatomic potential force can be approximated by treating the cluster as a single, larger node. The *theta* parameter determines the accuracy of the approximation: if the ratio *w* / *l* of the width *w* of the quadtree cell to the distance *l* from the node to the cell’s center of mass is less than *theta*, all nodes in the given cell are treated as a single node rather than individually.

<a name="lennardJonesPotential_distanceMin" href="#lennardJonesPotential_distanceMin">#</a> <i>lennardJonesPotential</i>.<b>distanceMin</b>([<i>distance</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/lennardJonesPotential.js)

If *distance* is specified, sets the minimum distance between nodes over which this force is considered. If *distance* is not specified, returns the current minimum distance, which defaults to 0.85. A minimum distance establishes an upper bound on the strength of the force between two nearby nodes, avoiding instability. In particular, it avoids an infinitely-strong force if two nodes are exactly coincident; in this case, the direction of the force is random.

<a name="lennardJonesPotential_distanceMax" href="#lennardJonesPotential_distanceMax">#</a> <i>lennardJonesPotential</i>.<b>distanceMax</b>([<i>distance</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/lennardJonesPotential.js)

If *distance* is specified, sets the maximum distance between nodes over which this force is considered. If *distance* is not specified, returns the current maximum distance, which defaults to infinity. Specifying a finite maximum distance improves performance and produces a more localized layout.

<a name="lennardJonesPotential_repulsivePower" href="#lennardJonesPotential_repulsivePower">#</a> <i>lennardJonesPotential</i>.<b>repulsivePower</b>([<i>n</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/lennardJonesPotential.js)

If *n* is specified, sets the repulsive exponent in the generalized Lennard-Jones potential, `(n*r^-m - m*r^-n)/(m-n)`. If *n* is not specified, returns the current repulsive exponent, which defaults to 12.

<a name="lennardJonesPotential_attractivePower" href="#lennardJonesPotential_attractivePower">#</a> <i>lennardJonesPotential</i>.<b>attractivePower</b>([<i>m</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/lennardJonesPotential.js)

If *m* is specified, sets the attractive exponent in the generalized Lennard-Jones potential, `(n*r^-m - m*r^-n)/(m-n)`. If *m* is not specified, returns the current attractive exponent, which defaults to 6.

#### Central-Body

The [Central-Body](https://en.wikipedia.org/wiki/Classical_central-force_problem) force is a general-purpose interatomic potential force used in molecular dynamics which applies mutually amongst all [nodes](#simulation_nodes). This implementation accepts an interatomic potential function, and uses quadtrees and the [Barnes–Hut approximation](https://en.wikipedia.org/wiki/Barnes–Hut_simulation) to greatly improve performance; the accuracy can be customized using the [theta](#manyBody_theta) parameter.

Unlike links, which only affect two linked nodes, the interatomic potential force is global: every node affects every other node, even if they are on disconnected subgraphs.

<a name="centralPotential" href="#forceCentralPotential">#</a> d3.<b>forceCentralPotential</b>() · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/centralPotential.js)

Creates a new central-body force with the default parameters.

<a name="centralPotential_strength" href="#centralPotential_strength">#</a> <i>centralPotential</i>.<b>strength</b>([<i>strength</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/centralPotential.js)
...
<a name="centralPotential_theta" href="#centralPotential_theta">#</a> <i>centralPotential</i>.<b>theta</b>([<i>theta</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/centralPotential.js)

If *theta* is specified, sets the Barnes–Hut approximation criterion to the specified number and returns this force. If *theta* is not specified, returns the current value, which defaults to 0.25.

To accelerate computation, this force implements the [Barnes–Hut approximation](http://en.wikipedia.org/wiki/Barnes–Hut_simulation) which takes O(*n* log *n*) per application where *n* is the number of [nodes](#simulation_nodes). For each application, a [quadtree](https://github.com/d3/d3-quadtree) stores the current node positions; then for each node, the combined force of all other nodes on the given node is computed. For a cluster of nodes that is far away, the interatomic potential force can be approximated by treating the cluster as a single, larger node. The *theta* parameter determines the accuracy of the approximation: if the ratio *w* / *l* of the width *w* of the quadtree cell to the distance *l* from the node to the cell’s center of mass is less than *theta*, all nodes in the given cell are treated as a single node rather than individually.

<a name="centralPotential_distanceMin" href="#centralPotential_distanceMin">#</a> <i>centralPotential</i>.<b>distanceMin</b>([<i>distance</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/centralPotential.js)

If *distance* is specified, sets the minimum distance between nodes over which this force is considered. If *distance* is not specified, returns the current minimum distance, which defaults to 0.85. A minimum distance establishes an upper bound on the strength of the force between two nearby nodes, avoiding instability. In particular, it avoids an infinitely-strong force if two nodes are exactly coincident; in this case, the direction of the force is random.

<a name="centralPotential_distanceMax" href="#centralPotential_distanceMax">#</a> <i>centralPotential</i>.<b>distanceMax</b>([<i>distance</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/centralPotential.js)

If *distance* is specified, sets the maximum distance between nodes over which this force is considered. If *distance* is not specified, returns the current maximum distance, which defaults to infinity. Specifying a finite maximum distance improves performance and produces a more localized layout.

<a name="centralPotential_potentialFunction" href="#centralPotential_potentialFunction">#</a> <i>centralPotential</i>.<b>potentialFunction</b>([<i>f</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/centralPotential.js)

If *f* is specified, sets the interatomic potential function `f(r)`. If *f* is not specified, returns the current interatomic potential function, which defaults to:
```js
rho => Math.pow(rho,-12)-2*Math.pow(rho,-6)
```
<a name="centralPotential_gradientFunction" href="#centralPotential_gradientFunction">#</a> <i>centralPotential</i>.<b>gradientFunction</b>([<i>fp</i>]) · [Source](https://github.com/gvarnavi/d3-force-md/blob/master/src/centralPotential.js)

If *fp* is specified, sets the interatomic potential gradient function `f'(r)/r`. If *fp* is not specified, returns the current interatomic potential gradient function, which defaults to:
```js
rho => 12*(Math.pow(rho,6)-1)/Math.pow(rho,14)
```
